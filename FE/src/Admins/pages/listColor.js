...
function ListColor() {
    ...
    const HandleDelete = async (id) =>{
        ...
        await ColorService.delete(id).then((response) =>{
            if(response?.status === 200) {setColors(colors.filter((x) => x.id !== id)); Notification.close();}
